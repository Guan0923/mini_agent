...
from cloud.auth.types import AuthStorageUnavailable, UserIdentity
...
class PostgresAuthRepository:
    ...
    def initialize(self) -> None:
        with self.connection() as connection:
            ...
            self._validate_account_ids(connection)

    @staticmethod
    def _validate_account_ids(connection: psycopg.Connection[dict[str, Any]]) -> None:
        rows = connection.execute("SELECT id FROM users WHERE kind='account'").fetchall()
        invalid = []
        for row in rows:
            value = str(row["id"])
            try:
        ...
        if invalid:
            raise RuntimeError(
                "Cloud account IDs require the canonical UUID form; "
                "run scripts/reset_storage_architecture.py to inspect the existing data before upgrading."
            )
    ...
    def _identity(row: dict[str, Any]) -> UserIdentity:
        return UserIdentity(
            str(row["id"]),
            str(row["email"]) if row.get("email") is not None else None,
            str(row.get("identity_kind") or row.get("kind") or "account"),
        )
    ...
    def user_by_email(self, email: str) -> UserIdentity | None:
        with self.connection() as connection:
            row = connection.execute(
                "SELECT id,email,kind FROM users WHERE email=%s AND kind='account'", (email,)
            ).fetchone()
        return self._identity(row) if row else None

    def user_by_id(self, user_id: str) -> UserIdentity | None:
        with self.connection() as connection:
            row = connection.execute("SELECT id,email,kind FROM users WHERE id=%s", (user_id,)).fetchone()
        return self._identity(row) if row else None
    ...
    def create_guest(self) -> UserIdentity:
        ...
        with self.connection() as connection:
            connection.execute(
                "INSERT INTO users(id,email,password_hash,created_at,kind) VALUES (%s,NULL,'',%s,'guest')",
                (user_id, now),
            )
        return UserIdentity(user_id, None, "guest")
    ...
    def create_guest_session(self, user_id: str, *, ttl_seconds: int = 2_592_000) -> str:
        ...
        with self.connection() as connection:
            connection.execute(
                """INSERT INTO users(id,email,password_hash,created_at,kind)
                VALUES (%s,NULL,'',%s,'guest')""",
                (user_id, now),
            )
            connection.execute(
                """INSERT INTO auth_sessions
                (id,user_id,kind,token_hash,created_at,expires_at,last_seen_at)
                VALUES (%s,%s,'browser',%s,%s,%s,%s)""",
                (uuid4().hex, user_id, self._token_hash(token), now, now + ttl_seconds, now),
            )
        return token

    def delete_guest(self, user_id: str) -> None:
        with self.connection() as connection:
            connection.execute("DELETE FROM users WHERE id=%s AND kind='guest'", (user_id,))
    ...
    def register_user(self, email: str, code: str, password: str) -> UserIdentity:
        ...
        try:
            with self.connection() as connection:
                ...
                connection.execute(
                    "INSERT INTO users(id,email,password_hash,created_at,kind) VALUES (%s,%s,%s,%s,'account')",
                    (user_id, email, password_hash, now),
                )
        except psycopg.errors.UniqueViolation as exc:
            raise ValueError("该邮箱已注册，请直接登录。") from exc
        return UserIdentity(user_id, email, "account")

    def authenticate(self, email: str, password: str) -> UserIdentity | None:
        with self.connection() as connection:
            row = connection.execute(
                "SELECT id,email,password_hash,kind FROM users WHERE email=%s AND kind='account'", (email,)
            ).fetchone()
    ...
    def reset_password(self, email: str, code: str, password: str) -> UserIdentity:
        ...
        with self.connection() as connection:
            ...
            if not self._consume_challenge(connection, email, "reset", code):
                raise ValueError("验证码无效或已过期。")
            row = connection.execute(
                "SELECT id,email,kind FROM users WHERE email=%s AND kind='account' FOR UPDATE", (email,)
            ).fetchone()
            if row is None:
                raise ValueError("验证码无效或已过期。")
    ...
    def resolve_token(self, token: str) -> tuple[UserIdentity, str] | None:
        ...
        with self.connection() as connection:
            row = connection.execute(
                """SELECT s.kind,u.id,u.email,u.kind AS identity_kind
    ...
    def set_pending_guest_import(self, user_id: str, guest_id: str) -> None:
        ...
        with self.connection() as connection:
            connection.execute(
                ...
                ON CONFLICT(target_user_id) DO UPDATE SET guest_user_id=CASE WHEN guest_imports.status='pending' THEN EXCLUDED.guest_user_id ELSE guest_imports.guest_user_id END,
                status=guest_imports.status,updated_at=guest_imports.updated_at""",
    ...
    def pending_guest_import(self, user_id: str) -> dict[str, object] | None:
        with self.connection() as connection:
            row = connection.execute(
                "SELECT guest_user_id,status,created_at,updated_at FROM guest_imports WHERE target_user_id=%s",
                (user_id,),
            ).fetchone()
    ...
    def finish_guest_import(self, user_id: str, decision: str) -> None:
        ...
        with self.connection() as connection:
            connection.execute(
                "UPDATE guest_imports SET status=%s,updated_at=%s WHERE target_user_id=%s AND status='pending'",
