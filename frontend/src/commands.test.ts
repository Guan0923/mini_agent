...
describe("web command catalog", () => {
  it("rejects unsupported commands at browser entry points", () => {
    const removed = ["/new", "/permission", "/sessions", "/fork", "/benchmark", "/history", "/clear", "/tools", "/agent", "/plan", "/resume", "/time", "/display"];
  ...
  it("parses retained commands and documents only the browser catalog", () => {
    ...
    expect(parseCommand("/help anything")).toBeNull();
    ...
    expect(HELP_TEXT).not.toContain("/new");
