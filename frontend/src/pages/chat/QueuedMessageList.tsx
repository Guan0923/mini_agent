      ...
      <ol>
        {items.map((item, index) => (
          <li key={item.id} className="queued-message-item">
            ...
            <span className="queued-message-content">{item.content}</span>
            <span className="queued-message-actions">
              <button type="button" className="queued-message-button" aria-label={`发送第 ${index + 1} 条待发送消息`} onClick={() => onSend(item)}>
                <SendOutlined aria-hidden="true" />
              </button>
              <button type="button" className="queued-message-button" aria-label={`编辑第 ${index + 1} 条待发送消息`} onClick={() => onEdit(item)}>
                <EditOutlined aria-hidden="true" />
              </button>
              <button type="button" className="queued-message-button danger" aria-label={`删除第 ${index + 1} 条待发送消息`} onClick={() => onDelete(item)}>
