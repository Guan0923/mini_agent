...
export interface ProjectAgentsInitResult {
  created: true;
  path: "AGENTS.md";
  content: string;
  byte_count: number;
...
export async function initProjectAgents(projectId: string): Promise<ProjectAgentsInitResult> {
  return requestJson<ProjectAgentsInitResult>(`/api/projects/${encodeURIComponent(projectId)}/agents/init`, {
    method: "POST",
  });
}
