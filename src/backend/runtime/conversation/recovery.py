...
def reconstruct_attempt(state: RuntimeState) -> tuple[RuntimeState, RuntimeState]:
    ...
    if old_run.plan is not None:
        for step in old_run.plan.steps:
            if step.tool_message.call_id in indeterminate:
                step.status = "indeterminate"
                step.result = "Tool outcome is indeterminate after process interruption."
            elif step.status == "running":
                step.status = "failed"
                step.result = "Tool was not executed before the previous process stopped."
    ...
    if source_run.plan is not None and old_run.plan is not None:
        resumed_steps = {step.id: step for step in old_run.plan.steps}
        for step in source_run.plan.steps:
            replacement = resumed_steps.get(step.id)
            if replacement is not None:
