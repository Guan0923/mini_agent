...
def test_mcp_definitions_match_the_safe_registry_specs(tmp_path: Path) -> None:
    ...
    assert [tool.name for tool in definitions] == ["get_current_time", "read_file", "glob", "grep", "read_pdf"]
    safe_specs = {
...
def test_mcp_server_registers_the_safe_tool_definitions(tmp_path: Path) -> None:
    ...
    assert [tool.name for tool in result.root.tools] == ["get_current_time", "read_file", "glob", "grep", "read_pdf"]
