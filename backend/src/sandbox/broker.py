...
class WindowsBrokerClient:
    ...
    def from_system(cls, *, is_windows: bool | None = None) -> WindowsBrokerClient:
        ...
        source_boundary = Path(__file__).resolve().parents[3]
        runtime_root = Path(sys.executable).resolve().parent.parent
        runtime_access: dict[str, Path] = {}
        try:
            if runtime_root.is_relative_to(source_boundary):
                runtime_access = {
                    "service_runtime_path": runtime_root,
                    "service_runtime_boundary_path": source_boundary,
                }
        except OSError:
            # A system Python installation already has service-readable ACLs;
            # only a checkout-local virtual environment needs an explicit grant.
            pass
        base_runtime = Path(sys.base_prefix).resolve()
        if base_runtime != runtime_root:
            runtime_access.update(
                {
                    "service_base_runtime_path": base_runtime,
                    "service_base_runtime_boundary_path": base_runtime.parent,
                }
            )
