...
class WindowsServiceInstaller:
    ...
    def __init__(
        ...
        *,
        ...
        service_runtime_path: Path | None = None,
        service_runtime_boundary_path: Path | None = None,
        service_base_runtime_path: Path | None = None,
        service_base_runtime_boundary_path: Path | None = None,
    ) -> None:
        ...
        self.service_runtime_path = Path(service_runtime_path) if service_runtime_path is not None else None
        self.service_runtime_boundary_path = (
            Path(service_runtime_boundary_path) if service_runtime_boundary_path is not None else None
        )
        self.service_base_runtime_path = (
            Path(service_base_runtime_path) if service_base_runtime_path is not None else None
        )
        self.service_base_runtime_boundary_path = (
            Path(service_base_runtime_boundary_path) if service_base_runtime_boundary_path is not None else None
        )
        ...
        if (self.service_runtime_path is None) != (self.service_runtime_boundary_path is None):
            raise ValueError("service_runtime_path and service_runtime_boundary_path must be provided together")
        if (self.service_base_runtime_path is None) != (self.service_base_runtime_boundary_path is None):
            raise ValueError(
                "service_base_runtime_path and service_base_runtime_boundary_path must be provided together"
            )
    ...
    def _run_local_transaction(self, operation: str, backend_sid: str | None) -> None:
        ...
        if self.service_runtime_path is not None and self.service_runtime_boundary_path is not None:
            try:
                commands.extend(
                    grant.runner_command()
                    for grant in _source_acl_grants(
                        self.service_runtime_path, self.service_runtime_boundary_path, self.service_name
                    )
                )
            except ValueError as exc:
                raise BrokerInstallationError(
                    BrokerInstallFailureCode.ACL_FAILED,
                    "Broker Python 运行时权限配置失败，请以管理员权限重试。",
                ) from exc
        if self.service_base_runtime_path is not None and self.service_base_runtime_boundary_path is not None:
            try:
                commands.extend(
                    grant.runner_command()
                    for grant in _source_acl_grants(
                        self.service_base_runtime_path, self.service_base_runtime_boundary_path, self.service_name
                    )
                )
            except ValueError as exc:
                raise BrokerInstallationError(
                    BrokerInstallFailureCode.ACL_FAILED,
                    "Broker 基础 Python 运行时权限配置失败，请以管理员权限重试。",
                ) from exc
    ...
    def _run_elevated_transaction(self, operation: str, backend_sid: str | None) -> None:
        ...
        payload = {
            ...
            "service_runtime_path": str(self.service_runtime_path) if self.service_runtime_path is not None else None,
            "service_runtime_boundary_path": (
                str(self.service_runtime_boundary_path) if self.service_runtime_boundary_path is not None else None
            ),
            "service_base_runtime_path": (
                str(self.service_base_runtime_path) if self.service_base_runtime_path is not None else None
            ),
            "service_base_runtime_boundary_path": (
                str(self.service_base_runtime_boundary_path)
                if self.service_base_runtime_boundary_path is not None
                else None
            ),
        }
