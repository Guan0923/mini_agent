...
class SQLiteRuntimeMixin:
    ...
    def _auto_title_sidebar_thread(
        ...
        existing_nodes: list[RuntimeNode],
    ) -> None:
        """Name a new main Thread from its first persisted user text."""

        if node.thread_id != node.session_id or any(
            isinstance(item, TreeRuntimeState) and item.thread_id == node.thread_id for item in existing_nodes
        ):
            return
