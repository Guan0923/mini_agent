...
DEFAULT_MEMORY_CONFIG: dict[str, object] = MemorySettings().to_dict()
...
def normalize_memory_config(current: Mapping[str, object], values: Mapping[str, object]) -> dict[str, object]:
    """Merge and validate the opt-in per-user Memory configuration."""

    merged = dict(DEFAULT_MEMORY_CONFIG)
    merged.update(current)
    merged.update(values)
    if values.get("generate_memories") is False:
        merged["automatic_memory_enabled"] = False
    return MemorySettings.from_mapping(merged).to_dict()
