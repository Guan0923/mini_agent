...
class AuthSettingsMixin:
    ...
    def settings_for_user(self, user_id: str, *, email: str = "") -> dict[str, object]:
        ...
        return {
            ...
            "memory_config": self.memory_config_for_user(user_id),
    ...
    def memory_config_for_user(self, user_id: str) -> dict[str, object]:
        config = self._config(user_id)
        ...
        try:
            ...
            # Memory is optional. A hand-edited invalid section fails closed
            # instead of preventing the user from starting an ordinary chat.
            return dict(DEFAULT_MEMORY_CONFIG)

    def update_memory_config(self, user_id: str, values: Mapping[str, object]) -> dict[str, object]:
        current = self.memory_config_for_user(user_id)
        result = normalize_memory_config(current, values)
        config_store = self._config_store(user_id)
        if config_store is None:
            raise ValueError("memory settings require a local config store")
        config_store.update({"memory": result})
        return result
