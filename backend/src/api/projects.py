...
from backend.runtime.agent_init import AgentInitError, initialize_project_agents
...
@router.post("/projects/{project_id}/agents/init")
def initialize_project_agents_file(
    project_id: str,
    request: Request,
    identity: UserIdentity = Depends(require_user),
) -> dict[str, object]:
    """Create the current project's root ``AGENTS.md`` without overwriting."""
    ...
    try:
        result = initialize_project_agents(Path(project.cwd))
