...
def _payload(item, state: WebAppState | None = None) -> dict[str, object]:
    payload = item.to_dict()
    if state is not None:
        project = state.projects.session_project(item.session_id)
...
def _scope_items(state: WebAppState, items: list[Any], project_id: str | None) -> list[Any]:
    return [item for item in items if _project_id(state, item.session_id) == project_id]


def _complete_scope_order(state: WebAppState, items: list[Any], project_id: str | None) -> list[str]:
    known = {item.thread_id for item in items}
    stored = [item for item in state.projects.sidebar_thread_order(project_id) if item in known]
    stored_ids = set(stored)
    missing = sorted(
        (item for item in items if item.thread_id not in stored_ids),
        key=lambda item: (item.created_at, item.thread_id),
        reverse=True,
    )
    return [item.thread_id for item in missing] + stored


def _active_scope_order(state: WebAppState, store, project_id: str | None) -> tuple[list[Any], list[str], list[str]]:
    scope_items = _scope_items(state, store.list_sidebar_threads(state="all"), project_id)
    active = [item for item in scope_items if item.state == "active"]
    full_order = _complete_scope_order(state, scope_items, project_id)
    active_ids = {item.thread_id for item in active}
...
def list_sidebar_threads(
    ...
    items = store.list_sidebar_threads(state=state)
    if state == "active":
        all_items = store.list_sidebar_threads(state="all")
        groups: dict[str | None, list[Any]] = {}
        for item in items:
            groups.setdefault(_project_id(web, item.session_id), []).append(item)
        ordered: list[Any] = []
        for project_id, group in groups.items():
            by_id = {item.thread_id: item for item in group}
        ...
        items = ordered
    return [_payload(item, web) for item in items]
...
def update_sidebar_thread_order(
    ...
    current_ids = {item.thread_id for item in active}
    if body.ordered_thread_ids is not None:
        ...
        by_id = {item.thread_id: item for item in active}
        if body.sort_by == "created_at":
            next_order = sorted(
                current_order,
                key=lambda thread_id: (by_id[thread_id].created_at, thread_id),
                reverse=True,
            )
        else:
            next_order = sorted(
                ...
                key=lambda thread_id: (
                    by_id[thread_id].last_activity_at,
                    by_id[thread_id].created_at,
                    thread_id,
                ),
                reverse=True,
            )
...
def rename_sidebar_thread(
    ...
    return _payload(store.update_sidebar_thread(thread_id, title=body.title.strip(), title_is_custom=True), web)
...
def archive_sidebar_thread(thread_id: str, request: Request) -> dict[str, object]:
    ...
    return _payload(store.update_sidebar_thread(thread_id, archived_at=utc_now(), deleted_at=None), web)
...
def restore_sidebar_thread(thread_id: str, request: Request) -> dict[str, object]:
    ...
    store = session_store(web)
    _require(store, thread_id)
    return _payload(store.update_sidebar_thread(thread_id, archived_at=None, deleted_at=None), web)
...
def delete_sidebar_thread(thread_id: str, request: Request) -> dict[str, object]:
    ...
    return _payload(store.update_sidebar_thread(thread_id, deleted_at=utc_now()), web)
