...
from backend.storage.memory import MemoryStorageError, MemoryStore
...
class MemoryConfigPayload(BaseModel):
    use_memories: StrictBool = False
    generate_memories: StrictBool = False
    automatic_memory_enabled: StrictBool = False
...
def update_memory_config(
    body: MemoryConfigPayload,
    request: Request,
    identity: Annotated[UserIdentity, Depends(require_user)],
) -> dict[str, object]:
    _origin_guard(request)
    try:
        result = request.app.state.web.settings.update_memory_config(
            identity.id,
            body.model_dump(exclude_unset=True),
        )
        if result["use_memories"] or result["generate_memories"]:
            MemoryStore(request.app.state.web.user_paths(identity.id)).ensure()
        if result["generate_memories"]:
            request.app.state.web.memory_automation.wake()
        else:
            request.app.state.web.memory_automation.stop_user(identity.id)
        return result
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    except (MemoryStorageError, OSError, sqlite3.Error) as exc:
        raise HTTPException(status_code=503, detail="Memory storage is temporarily unavailable.") from exc
