...
from backend.runtime.memory import MemoryDiagnosticsRegistry
...
class WebAppState:
    ...
    def __init__(
        ...
        *,
        ...
        memory_model_factory: Callable[[str], object] | None = None,
    ) -> None:
        ...
        from backend.runtime.memory import MemoryAutomationService, ProviderMemoryModel

        factory = memory_model_factory or (lambda user_id: ProviderMemoryModel(self.model_config_for_user(user_id)))
        self.memory_automation = MemoryAutomationService(self, factory)
        self._started = False

    def start(self) -> None:
        """Start process-owned background services exactly once."""
