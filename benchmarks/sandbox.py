...
MCP_SERVER_NAME_PATTERN = "bench_demo"
...
class Sandbox:
    ...
    def materialize_workspace(self, task: BenchmarkTask) -> Path:
        """Write the task's seed files, skills, and MCP config into a fresh directory."""
        workspace = self.workspaces_dir / f"{task.name}-{time.time_ns()}"
        workspace.mkdir(parents=True, exist_ok=False)
        for seed_file in task.seed.files:
            path = workspace / seed_file.path
