...
These paths are ignored by Git. `LOG_FULL_MESSAGES=true` is the development default; set it to `false` in `.env` to write summaries instead of complete audit-message bodies. Sensitive key names and values are redacted in both modes. Wire payloads are retained without authentication headers; stream responses are stored as ordered JSON events. SQLite data is not imported; after verifying PostgreSQL, remove only the legacy `.mini_agent/checkpoints.db` file if it is no longer needed. Do not use production credentials or sensitive personal data in checked-in fixtures.
